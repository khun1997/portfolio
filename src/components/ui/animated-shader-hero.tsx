...
void main(){gl_Position=position;}`;
    ...
    init() {
      ...
      (program as any).theme = gl.getUniformLocation(program, "u_theme");
    }
...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		col=mix(col, mix(
			vec3(bg*.25, bg*.137, bg*.05),
			vec3(bg*.85, bg*.65, bg*.4),
			u_theme
		), d);
	}
	// Lighten overall for light mode
